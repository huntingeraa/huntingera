...
const emailSend = asyncHandler(async (req, res) => {
 ...
 let otpData= new Otp({
  email: 'producthunting5214@gmail.com',
  code: otpcode,
...
const emailSucc = asyncHandler(async (req, res) => {
  ...
  let dataa = await Admin.findOne({email: 'producthunting5214@gmail.com'});
  const response={}
  if(data){
...
const mailer=(email, otp)=>{
  ...
  let mailTransporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
          user: 'huntingeraa@gmail.com',
          pass: 'hsjfeaecvkhwbmfq'
      }
  });
   
  let mailDetails = {
      from: 'huntingeraa@gmail.com',
      to: email,
      subject: 'Your Hunting Era Admin Otp is: ',
